...
実行テンプレート：0.12.1。データschema：0.2（任意項目で後方互換を維持）。共通UX仕様：0.5 candidate（改善後のv1.0採用は別途検証）。

0.12.1では地域診断の各テーマ見出しに選択地域を明示し、「各指標の最新年」を選んだ場合は指標ごとに採用可能な最新年を表示する。独自の広域区分で地域全体の公表値や承認済み集計値がない場合は、同一年の国・地域別データの件数・最小・最大・年度を示す。国別の率を広域値として単純平均しない。

アジア全域の入口は、採用済みの世界ポートフォリオからUN M49 Asia (142) の50か国・地域と5広域を切り出して生成できる。国際系列の利用可能な年・値を各地域で保持し、国別Censusの所在情報と国内統計の採用状態を分ける。アジア50か国の国内地域版が完成したという意味ではない。[範囲・収録監査](docs/ASIA_SCOPE_AND_STATUS.md)を参照。

世界・大陸・広域では、国連WPPの人口・年齢・男女別系列と、採用済みの国連SDG国際系列を別の読み込みファイルから表示する。年齢構成は2000年（グレースケール）と選択年を同じ尺度の人口ピラミッドで比較し、男女構成は円グラフ、割合指標は0～100%の帯、年次推移は折れ線、地域差は地図と全件表で示す。国を超える画面に国勢調査の異なる年・定義の数値を合算して載せない。国以下の国勢調査・地方統計は国別の収集成果を順次接続する。取得範囲と未収録範囲は[国際データの版記録](docs/WORLD_INTERNATIONAL_PORTFOLIO.md)で確認できる。

共通指標にはGDP総額・1人当たりGDP・実質GDP、インフレ率、失業率・若年NEET率、故意の殺人率を加えた。[収集・比較の契約](docs/ECONOMY_SAFETY_EDUCATION_PORTFOLIO.md)を参照。就学率と地域全体のGini係数は、定義と公式広域値を確認してから採用する。
...
0.12.0では各指標の地域内比較に、比較可能な構成地域の件数、最小・最大の地域名と値、差を表示する。同名地域はコードで区別し、割合の差はパーセントポイントとする。これは取得済みの共通指標にも適用される。インフレ率の広域値がIMFの区分と一致しない場合、独自の平均値は作らず欠測のままとする。
...
国別のカバー拡張で確認した公開source所在は、[`config/kit-source-feedback-selections.json`](config/kit-source-feedback-selections.json)に選び、`npm run sources:export:kit-feedback`で[`evidence/KIT_SOURCE_FEEDBACK.json`](evidence/KIT_SOURCE_FEEDBACK.json)へ書き出す。Kit側でdry-run、取込、検証してから両方のcommitを記録する。これは資料の所在を戻す経路であり、数値やAreaDataでの採用判定をKitへ自動移植しない。独立監査前の候補は所在確認段階として扱う。Bangladesh・Laos・Ugandaの原本再現とセル照合の実施範囲・未了部分は[３カ国source監査記録](docs/evidence/three-country-source-replay-audit-2026-09-24.md)に分けて示す。
...
CIはWindows/Linux、Node 22/24で検証します。初期収集の結果は[0.2検証記録](docs/VALIDATION_v0.2.md)、上位再選択等は[0.2.1検証記録](docs/VALIDATION_v0.2.1.md)、計画・資料機能は[0.3検証記録](docs/VALIDATION_v0.3.md)、世界入口・指標別内部比較・診断出力は[0.4検証記録](docs/VALIDATION_v0.4.md)、共通sourceと国別所在台帳は[0.4.1検証記録](docs/VALIDATION_v0.4.1.md)、中米7か国・完全被覆集計・MariaDB基礎は[0.5検証記録](docs/VALIDATION_v0.5.md)、ベリーズ・グアテマラ国勢調査の実収集と部分統合は[0.6検証記録](docs/VALIDATION_v0.6.md)、7か国の公式人口・国内階層・混合基準年集計は[0.7検証記録](docs/VALIDATION_v0.7.md)、UN人口比較・国勢調査リンク・3言語UIは[0.8検証記録](docs/VALIDATION_v0.8.md)、Census採用年地図・調査履歴は[0.9検証記録](docs/VALIDATION_v0.9.md)、国別制作の完成工程は[0.9.1検証記録](docs/VALIDATION_v0.9.1.md)、独立完成監査工程は[0.9.2検証記録](docs/VALIDATION_v0.9.2.md)、アメリカ大陸入口は[0.10検証記録](docs/VALIDATION_v0.10.md)、撤回された旧判定は[0.10.1検証記録](docs/VALIDATION_v0.10.1.md)、厳格化した再構築は[0.10.2検証記録](docs/VALIDATION_v0.10.2.md)、Kit v1.8.1世界Census所在台帳の統合は[0.10.3検証記録](docs/VALIDATION_v0.10.3.md)、地域要約と初回読込の軽量化は[0.10.4検証記録](docs/VALIDATION_v0.10.4.md)に保存します。42の受入シナリオは案件の採用範囲に応じて検証し、シナリオの追加を合格件数と扱いません。
