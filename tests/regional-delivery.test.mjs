...
const completeMatrix=(disposition='integrated')=>{
  ...
  for(const domain of COMPLETION_DOMAINS.filter(id=>id!=='semantic_table_column_inventory'))country[domain]={identified:true,accessed:true,acquired:true,inspected:true,geography_matched:true,adopted:true,completion_verified:true,urls:['https://example.org'],note:'Verified.'};
  const records=CENSUS_THEMES.map((theme,index)=>({country_area_id:'AAA',source_id:'source',table_id:'table',field_id:`field-${index}`,theme,disposition,reason:'Reviewed.',coverage_complete:true,country_edition_eligible:disposition==='integrated'}));
  return {matrix:buildCountryCompletionMatrix({scope_id:'M49:019',countries:[country]},{semanticInventory:{records},generatedAt:'2026-09-18T00:00:00Z'}),records};
};
test('regional publication gate accepts Americas only after every country edition domain and theme is integrated',async()=>{
