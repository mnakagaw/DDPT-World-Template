...
async function fixture(audit='pending'){
  ...
  await writeFile(path.join(root,'evidence/DELIVERY.json'),JSON.stringify({schema_version:'0.1',scope:{country_area_count:1},checks,independent_audit:{status:audit,file:'evidence/INDEPENDENT_AUDIT.md'},release:{public_status:'not_published'}}));return root;
}
...
test('regional publication gate accepts independently accepted candidate',async()=>{const root=await fixture('accept');const result=await verifyRegionalDelivery(root,{requirePublishable:true});assert.equal(result.ok,true);assert.equal(result.publishable,true);});
