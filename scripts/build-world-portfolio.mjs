...
export function buildWorldPortfolio(world,americas,wpp,sdg=null){
  ...
  if(sdg){if(sources.has(sdg.source.id))throw new Error('Duplicate SDG source');sources.set(sdg.source.id,sdg.source);}
  ...
  for(const indicator of sdg?.indicators||[]){if(indicators.has(indicator.id))throw new Error(`Duplicate SDG indicator ${indicator.id}`);indicators.set(indicator.id,indicator);}
  const observations=[...world.observations,...americas.observations.filter(row=>(localIds.has(row.territory_id)||countryRootIds.has(row.territory_id))&&!oldWppIds.has(row.indicator_id)&&!world.indicators.some(item=>item.id===row.indicator_id)),...wpp.observations,...(sdg?.observations||[])];
  const keys=new Set();
  ...
  const portfolioIds=[...wpp.indicators,...(sdg?.indicators||[])].map(item=>item.id);
  ...
  return {
    ...
    analysis:{...world.analysis,kind:'world',comparisons:[...comparisons.values()],terminal_territory_ids:[...terminals],
      default_period_by_indicator:{...world.analysis.default_period_by_indicator,...americas.analysis.default_period_by_indicator,...Object.fromEntries(portfolioIds.map(id=>[id,'2026']))},
...
export async function writeWorldPortfolio({world,americas,wpp,sdg,out}={}){
  ...
  const dataset=buildWorldPortfolio(await readJson(world),await readJson(americas),await readJson(wpp),sdg?await readJson(sdg):null);
  ...
  await writeFile(path.join(outDir,'HANDOFF.md'),`# AreaData worldwide international portfolio\n\nUN WPP 2024 Rev.1 supplies world, matching M49 regions and ${dataset.analysis.coverage.un_wpp_country_area_count}/${dataset.analysis.coverage.country_area_count} country/area rows for 2000 and 2023–2026. 2000 and selected-year age-sex profiles are normalized from separate UN male/female five-year age workbooks. ${sdg?'Seven fixed-slice social and services series are drawn from the official 2026 Q2.2 SDG archive; its custodian agencies are recorded per observation. Other themes and missing area-series cells remain uncollected.':'Other United Nations thematic source families remain uncollected in this edition.'}\n\nThe browser receives international observations and age profiles in data/supranational.json. Domestic Census country-depth evidence is in data/countries/*.json. Canonical data/dashboard.json retains source identity, provenance and records for audit.\n`);
...
if(isMain(import.meta.url)){
  try{const args=parseArgs(process.argv.slice(2),['world','americas','wpp','sdg','out']);if(!args.world||!args.americas||!args.wpp)throw new Error('Use --world file --americas file --wpp file [--sdg file] --out new-directory');await writeWorldPortfolio(args);}catch(error){reportError(error);}
}
