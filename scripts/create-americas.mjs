...
export async function createAmericas({out,sourceDir,centralAmericaProject,centralAmericaRaw,unWppFile,startYear,endYear,collect=collectWorld,generate=generateSite}={}){
  ...
  try{
    ...
    const dataset=buildAmericas(world,{centralAmerica});
...
if(isMain(import.meta.url)){
  try{const args=parseArgs(process.argv.slice(2),['out','source-dir','central-america-project','central-america-raw','un-wpp-file','start-year','end-year']);if(args.help)console.log('node scripts/create-americas.mjs --source-dir previous-world/raw --central-america-project previous-central-america --central-america-raw acquired-census-directory --un-wpp-file WPP.xlsx --out generated/americas');else await createAmericas({out:args.out,sourceDir:args['source-dir'],centralAmericaProject:args['central-america-project'],centralAmericaRaw:args['central-america-raw'],unWppFile:args['un-wpp-file'],startYear:args['start-year']===undefined?undefined:Number(args['start-year']),endYear:args['end-year']===undefined?undefined:Number(args['end-year'])});}catch(error){reportError(error);}
}
