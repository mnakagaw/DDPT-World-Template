...
Candidate R2 dataset SHA-256: `ec7f076ac69effcd5feae72a834561a877abcad3f02f2aebf76dd4f427eca776` at this checkpoint. It has five territories, zero polygons, 22 indicators, 352 observation records (40 domestic calculated-from-source-cell records and 312 distinct WDI national records), and zero planning documents. `validate-country` returned zero errors and one explicit no-local-planning-document warning; build and source-pinned five-area export verifier passed. `npm run check` and `npm test` (221/221) passed after the producer's provenance correction. Diagnostic CSV output has 110 rows for Bahrain and 22 rows per governorate, 198 rows total across five selected areas. The verifier checks all ten domestic indicators and all 40 within-area calculation labels in CSV, HTML and Markdown, plus thematic comparison, series and all-observations CSV classification. In the local browser, Bahrain displayed 1,501,635 and four of four comparable governorates, Capital displayed 548,345 as calculated from source values, and the planning page showed zero local materials. Selecting Capital and then Bahrain changed URL, title and selected-area values. The [independent audit](bahrain-independent-audit-2026-09-27.md) rejected the first fixed producer commit's P1 classification leak and independently accepted the narrow provenance remediation at `8942d851c9c2dcccec60a0de693db59577e3e8c3`. Full printed pages, downloaded file bytes, all 42 applicable scenarios and independent country acceptance remain open.
...
The independent first/R2 acquisitions were compared with `python scripts/compare-bahrain-census-replay.py --first generated/bahrain-areadata-20260926 --second generated/bahrain-census-r2-20260926`: all 45 substantive source table bodies and all 40 `(area, indicator, period, value)` tuples matched. The first run predates the provenance correction, so its calculated classification is not a reference. Exact response metadata, WDI refreshes and retrieval times are excluded from semantic equality. The fixed producer commit and pinned dataset passed an independent provenance-only recheck; full country `ACCEPT` remains outstanding. Next work: audit all remaining source fields and finer geography; reconcile legal code/boundary and municipality planning units; acquire individual approved plans, budgets, final accounts, implementation and evaluations; complete representative and exception area print/download scenarios and portal-term implementation. **Hosting/Public: not attempted.**
