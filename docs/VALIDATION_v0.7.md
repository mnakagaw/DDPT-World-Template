...
- 検証対象commit: `c3f938a6c45d6d3f77325ac73769d0c04677e6b0`（中米7か国の公式国勢調査人口アダプター）
- 検証日: 2026-09-15 JST
...
- 生成案件: `.work/areadata-ca-v0.7-census-final-3/`
...
| `node scripts/validate-country.mjs --project .work/areadata-ca-v0.7-census-final-3` | error 0、warning 10 |
| `git diff --check` | 合格 |
...
`http://127.0.0.1:4178/`の最終生成版をデスクトップのin-app browserで確認した。
...
| ID | 結果 | 今回の証拠・理由 |
|---|---|---|
| A01 | 合格 | El Salvadorのdepartment→municipality→district→同じ親を実画面確認。地図がない階層は代替表示 |
| A02 | 未実施 | planning資料が未収録 |
| A03 | 未実施 | 再読込は自動試験済みだが、実ブラウザの戻る・進む一式は未実施 |
| A04 | 未実施 | 全地域検索は実装済み。数千地域の実操作記録は未作成 |
| A05 | 合格 | 中米→7か国のKPI・地図・全件表を実画面照合 |
...
| A14 | 合格 | デスクトップ幅で上部選択、主要分析、次操作を実画面確認 |
...
| A21 | 合格 | district→同じmunicipalityをnative dropdownで再選択し、下位解除と指標・年保持を実画面確認 |
| A22 | 合格 | municipality→同じdepartmentをnative dropdownで再選択し、下位解除と親値への更新を実画面確認 |
...
- FTP公開と`areadata.net`公開確認は、この版では行っていない。

中米7か国の公式国勢調査人口レイヤーはローカル生成とGitHub保存の対象として合格した。AreaData全体、国内国勢調査の全テーマ、AreaPlanを完成扱いにはしていない。
