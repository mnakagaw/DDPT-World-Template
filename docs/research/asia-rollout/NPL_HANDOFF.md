...
Kit feedback is **source-location-only** (`official_location_identified`) until Kit independently audits each lead; it does not transfer raw originals, observations or country acceptance. AreaData and Kit commit IDs and validation will be entered in [Kit handoff](KIT_FEEDBACK_HANDOFF.md) after their respective pushes. Local: partial candidate. GitHub: scoped code/evidence pending commit. Hosting: none. Public: none.
