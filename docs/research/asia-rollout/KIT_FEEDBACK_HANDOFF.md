...
GitHub stores this partial country candidate's reproducible scripts and source decisions. The generated country site, raw originals and receipts remain local/ignored. The independent Georgia country verdict is `REJECT` while the isolated thematic-comparison P1 fix was accepted; Hosting/Public is not deployed. Kit must independently acquire, inspect, date and code-match each source before using values or advancing stages.

## Azerbaijan SSC age table, 2019 census and Astara planning-source locations

AreaData acquired 11 official originals and adopted only the 2026 SSC age/resident-estimate cells for 101 reporting rows plus three separate rounded-thousand Astara-city cells. The 2019 census ZIPs and 704 other XLS numeric columns remain unadopted; one Cabinet approval belongs only to Astara city. The [source audit](../../evidence/azerbaijan-ssc-2026-age-astara-planning-2026-09-26.md) records these limits. Kit receives 14 public locations and cautions, without values or originals.

| Step | Commit / artifact | Verification |
|---|---|---|
| AreaData producer | `e150eb27ee907f7b816911ad4ccea9b5cbaf1009` on `codex/asia-domestic-continuation-20260926` | Candidate SHA-256 `009c2a6e13a06a39f9d997818ea989f7582aaaa7b1b9a81c99381d9ac9ed324d`; fresh replay reproduced 11 original hashes and 2,023 adopted domestic tuples. Candidate/replay validators returned zero errors/warnings; builds, ten-area CSV/HTML and browser checks passed. AreaData check and 220/220 tests passed. Independent country [audit](../../evidence/azerbaijan-independent-audit-2026-09-26.md) returned `REJECT`, with no new P0/P1 implementation regression. |
| AreaData feedback bundle | `0ce513e38e071399f3e2b3a332b7bbb4f41f6a19`; bundle SHA-256 `5925fef1317b6a29ccb75b51c4ad90b2e82f68969e45b3f2d477a7f6fab522e9` | 130 public leads in 18 countries, 14 for Azerbaijan. All are capped at `official_location_identified`; no raw data or adopted values are transferred. |
...
The separate [Bahrain fixed-edition audit](../../evidence/bahrain-independent-audit-2026-09-27.md) rejected producer `41ddaafa` for a P1 display/export provenance error while confirming its acquired numeric cells. AreaData corrected the presentation at `8942d851c9c2dcccec60a0de693db59577e3e8c3`; a separate read-only checker accepted the narrow provenance remediation at the same dataset SHA, while keeping country `REJECT`. The nine public source-location leads and Kit import did not change. The feedback bundle's `origin_commit` remains the original source-discovery checkpoint and is not a country-acceptance statement.

## Qatar Census 2020 and planning-source location return

The [Qatar source account](../../evidence/qatar-census-2020-official-sources-2026-09-26.md) records the official 209-sheet Census 2020 workbook, selected direct municipal cells, the current GIS layer and QNMP planning leads. The local candidate has 153 adopted direct source cells but no verified 2020 polygons or adopted current municipal plans. All other workbook cells remain unassessed. Only six public URLs and reuse cautions were returned to Kit; no observations or raw originals were transferred.

| Step | Commit / artifact | Verification |
|---|---|---|
| AreaData producer | `31a8dad57f391219b5871f36e754a4c7b1aef76a` on `codex/asia-domestic-continuation-20260926` | Dataset SHA-256 `8ff8b9838c03ba5689c677afb28c616f1a6b61889b67a7bc3eb43828e6c1975f`; 9 areas, 17 domestic indicators and 153 direct cells. Validator zero errors/one planning warning, build, 9-area source-coordinate output check, browser recheck, AreaData check and 221/221 tests passed. Country acceptance remains pending. |
| AreaData full feedback bundle | `b29db36cfb5aa21624b136956793f68351e3d772`; `evidence/KIT_SOURCE_FEEDBACK.json` SHA-256 `beffd390afebc4ea000e6120b8740c77493782d2eae5c26aa89cc0daa2056b03` | 148 public leads in 20 countries, six for Qatar. All Qatar entries are capped at `official_location_identified`; `origin_commit` is the producer commit above. |
...
## Armenia post-freeze follow-up and Cyprus historical Census source locations

The [Armenia follow-up](../../evidence/armenia-areadata-partial-2026-09-27.md) added nine public official location leads outside its frozen dataset: boundary standards/availability, the Ministry community directory and workbook, three Syunik plan packages, and Kajaran annual plan/report. Its previously selected Armstat Census lead now says all 14 table responses were structurally decoded but semantic adoption remains limited. The [Cyprus partial candidate](../../evidence/cyprus-areadata-partial-2026-09-27.md) selected nine public leads from CYSTAT, DLS, the Ministry of Finance and municipal planning/finance sites. These are URLs and reuse cautions only. No Census cell, raw original, polygon, budget value, approval finding or country acceptance was transferred.

| Step | Commit / artifact | Verification |
|---|---|---|
| AreaData producer | `008064712056deac4b28ceaae2f099615262d837` on `codex/asia-domestic-continuation-20260926` | Armenia frozen dataset SHA-256 `75a744520166af7591a53ab5bc954dee5c771c821d8fd537c7d7ec8bfd74f2d2`; Cyprus partial dataset SHA-256 `40ed8299f8678f8ab8aadca3f82f40fe173f94b57a92ad7b67a9ca341bcd26f6`. Armenia validator 0 errors/warnings, Cyprus 0 errors/seven explicit planning/rights warnings; both sites built. AreaData check 149 modules/templates, test 224/224. Both country verdicts remain `REJECT`. |
| AreaData full feedback bundle | `evidence/KIT_SOURCE_FEEDBACK.json`, SHA-256 `76c3fae6af994f939b6c955c1e30a6abf562c75468dfee0ea3dc8bd6af5accf9` | 197 accumulated public locations, 26 Armenia and nine Cyprus; `origin_commit` is the producer commit. Every source is capped at `official_location_identified`. |
| Scoped transfer file | Ignored Cyprus evidence `KIT_SOURCE_FEEDBACK_ARM_CYP_FOLLOWUP_ONLY.json`, SHA-256 `6b09b5655ddba8b67874bf979fea34be476d3fab272cf8df627233c36dfa3415` | 19 records: nine new Armenia, nine new Cyprus, and one revised Armenia Census caution. The subset avoided updating unrelated prior Kit records. |
| Kit import | `1824ff36b452bc7f9054eb8df88db83c2ee8d5fc` on `codex/asia-source-feedback-qatar-20260927` | Dry-run and import accepted 19: 18 inserted, one updated, zero rejected. Repeat dry-run: 19 unchanged. Kit `npm run check`, `npm test` 173/173 and `npm run verify:kit` (`ready:true`) passed. All 26 Armenia and nine Cyprus records remain `official_location_identified` and `not_acquired_by_kit_preflight`. Kit branch push and remote SHA read-back matched. |

Neither country's local candidate was deployed to Hosting/Public. Cyprus still needs a coded current-authority crosswalk, full Census/sector review, legal boundary-date confirmation, source terms, plan bodies, actual saved exports, all applicable 42 scenarios and independent country acceptance; Armenia still lacks a joined dated polygon layer, broad semantic adoption and the same acceptance gates.
