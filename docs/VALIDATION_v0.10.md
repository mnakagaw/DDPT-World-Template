...
- 最終生成先: `.work/areadata-americas-v0.10.0-release-2026-09-18-final5`
...
独立監査のREJECTを受け、次の6点を修正して全項目を再監査する。
...
5. 原資料台帳から成果物外のWPP作業パスを除き、成果物内の同一原本へhashで一本化。341件すべてが成果物相対パスで実在することを検証するゲートを追加。
6. テーマ比較の地図・ランキング行の操作を、分析対象の選択から分離。比較内の注目だけを連動させ、上部の分析対象、URL、指標、資料、出力先を変更しない回帰検証を追加。
