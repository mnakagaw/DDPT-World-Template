...
現候補を別担当が独立監査した判定は `INCOMPLETE AUDIT`。中央アジアのテーマ診断の値と親地域への再選択は独立確認されたが、ブラウザー保存ファイルのバイト列、狭幅・キーボード、多言語とページ遷移の全導線、国内枝を使う国→市の操作は未確認または未実装である。`npm run verify:regional` は4項目の部分実施により `publishable:false` を返す。50件の国際統計入口と国内版50件の完成を区別する。公開ゲートが要求する `ACCEPT` まではFTP・公開サイトへ反映しない。候補の受入票と独立監査を含む証跡は `.work/areadata-asia-20260926-commit-c6cf9fe/evidence/` に保存している。
