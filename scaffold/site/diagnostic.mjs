...
export function renderInternalComparison(data,parentId,indicatorId,period,{interactive=true,language='en',suppressEmpty=false,printGeometryRegistry=null,printGeometryCache=null}={}) {
  ...
  const cached=!interactive&&printGeometryCache?.find(item=>item.features.length===comparison.features.length&&item.features.every((feature,index)=>feature===comparison.features[index]));
  const geometry=cached?.geometry||mapGeometry(comparison.features);
  if(!interactive&&printGeometryCache&&!cached)printGeometryCache.push({features:comparison.features,geometry});
  const byId=new Map(comparison.rows.map(row=>[row.area.id,row]));
  const label=`${comparison.set.label} · ${indicator.name} · ${period} · ${indicator.unit}`;
  const map=geometry.paths.length?`<svg class="internal-map" viewBox="0 0 760 400" role="group" aria-label="${e(label)}. ${interactive?'Inspect values without changing the analysis area.':'Values and sources are listed in the adjacent table.'}"><title>${e(label)}</title><rect width="760" height="400" fill="#f4f8f7"/>${geometry.paths.map(path=>{
    const row=byId.get(path.id),text=`${row.area.name}: ${valueText(data,row.value,indicator)} ${rowUnit(row,indicator)} · ${statusLabel(row.status,row.provenance)}${row.comparable?'':' · comparison not established'}`;
    if(!interactive&&printGeometryRegistry){
      let shapeId=printGeometryRegistry.get(path.d);
      if(!shapeId){shapeId=`diagnostic-shape-${printGeometryRegistry.size+1}`;printGeometryRegistry.set(path.d,shapeId);}
      return `<use href="#${shapeId}" fill="${e(colorForComparison(comparison,row))}" fill-rule="evenodd" class="internal-area" role="img" aria-label="${e(text)}"><title>${e(text)}</title></use>`;
    }
...
function seriesReport(data,area,indicator) {
  ...
  return `${graph}<table class="history-table"><thead><tr><th>Period</th><th>Value / unit</th><th>Status / meaning</th><th>Source</th></tr></thead><tbody>${series.map(row=>{
    ...
    return `<tr><td>${e(row.period)}</td><td>${e(valueText(data,observedValue(row),indicator))} ${e(meaning.unit)}</td><td>${e(statusLabel(row.provenance==='calculated'?'calculated':row.status,row.provenance))}<p>${e(meaningText(meaning))}</p>${row.provenance==='calculated'&&row.footnote?`<p>Calculation: ${e(row.footnote)}</p>`:''}<p>Observation boundary edition: ${e(observedBoundary(row))}.</p></td><td>${renderSourceAttribution(indicator,row,source,row.period)}<p>Retrieved ${e(source?.retrieved_at || 'not recorded')}.</p></td></tr>`;
  }).join('')}</tbody></table>`;
}
...
export function diagnosticMarkdown(data,territoryId,period) {
  ...
  for(const indicator of indicatorsForTerritorialScope(data,territoryId)) {
    ...
    lines.push(`## ${md(indicator.theme)} — ${md(indicator.name)}`,'',`Overall: ${md(valueText(data,result.value,indicator))} ${md(result.unit)} · ${md(statusLabel(result.status,result.provenance))} · ${md(result.observation?.period || effectivePeriod)}`,result.provenance==='areadata_calculated'?`Aggregation: ${md(result.note)} Components: ${md(result.components.map(item=>`${item.territory_id}@${item.period}`).join(', '))}. Missing areas: ${md(result.missing_ids.join(', ') || 'none')}. Covered subtotal: ${md(result.covered_value)}.`:result.provenance==='calculated'?`Calculation: ${md(result.note)}`:result.provenance==='source_reported'?'Source-reported exact-area observation.':'Explicit data gap.',...(sourceScopeNote(data,territoryId,indicator.id)?[md(sourceScopeNote(data,territoryId,indicator.id))]:[]),md(meaningText(result)),`Observation boundary edition: ${md(observedBoundary(result.observation))}.`,`Source: ${md(source?.name)} · ${md(source?.url)} · retrieved ${md(source?.retrieved_at)}`,'','### Acquired history','','| Period | Value | Unit | Status | Definition / population / method / comparison context | Observation boundary | Source / retrieved |','|---|---:|---|---|---|---|---|');
    for(const row of seriesFor(data,territoryId,indicator.id)) {
      ...
      lines.push(`| ${md(row.period)} | ${md(valueText(data,observedValue(row),indicator))} | ${md(meaning.unit)} | ${md(statusLabel(row.provenance==='calculated'?'calculated':row.status,row.provenance))} | ${md(meaningText(meaning))}${row.provenance==='calculated'&&row.footnote?` Calculation: ${md(row.footnote)}`:''} | ${md(observedBoundary(row))} | ${md(historySource?.url)} / ${md(historySource?.retrieved_at)} |`);
    }
    if(comparison.set.terminal)lines.push('','Internal comparison stops at this area; no lower areas are included.');
      ...
      for(const row of comparison.rows)lines.push(`| ${md(row.area.name)} / ${md(row.area.id)} | ${md(row.area.type)} / ${md(row.area.code_system)} / ${md(row.area.official_code || 'unverified')} / ${md(row.area.parent_id)} / ${md(row.area.boundary_version || 'unverified')} | ${md(valueText(data,row.value,indicator))} | ${md(rowUnit(row,indicator))} / ${md(row.period || effectivePeriod)} | ${md(statusLabel(row.status,row.provenance))} / ${row.comparable?'comparable':md(row.reason || 'not comparable')} | ${md(meaningText({...row,reason:''}))}${row.provenance==='calculated'&&row.note?` Calculation: ${md(row.note)}`:''} | ${md(observedBoundary(row.observation))} / ${md(row.boundary_reason || 'Matched geometry')} | ${md(row.source?.url)} / ${md(row.source?.retrieved_at)} |`);
...
export function diagnosticHtml(data,territoryId,period) {
  const area=data.territories.find(row=>row.id===territoryId);
  if(!area)throw new Error('Unknown diagnostic area');
  const printGeometryRegistry=new Map();
  const printGeometryCache=[];
  const sections=indicatorsForTerritorialScope(data,territoryId).map(indicator=>{
    ...
    return `<section><h2>${e(indicator.theme)} — ${e(indicator.name)}</h2><p><strong>Overall: ${e(valueText(data,result.value,indicator))} ${e(result.unit)}</strong> · ${e(statusLabel(result.status,result.provenance))} · ${e(result.observation?.period || effectivePeriod)}</p>${result.provenance==='calculated'&&result.note?`<p>Calculation: ${e(result.note)}</p>`:''}${sourceScopeNote(data,territoryId,indicator.id)?`<p class="notice">${e(sourceScopeNote(data,territoryId,indicator.id))}</p>`:''}<p>${e(meaningText(result))}</p><p>Observation boundary edition: ${e(observedBoundary(result.observation))}.</p><p>Source: ${sourceLink(result.source)} · Retrieved ${e(result.source?.retrieved_at || 'not recorded')}.</p><h3>Acquired history</h3>${seriesReport(data,area,indicator)}${renderInternalComparison(data,territoryId,indicator.id,effectivePeriod,{interactive:false,printGeometryRegistry,printGeometryCache})}</section>`;
  }).join('');
  const printGeometryDefs=printGeometryRegistry.size?`<svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" style="position:absolute;width:0;height:0;overflow:hidden" aria-hidden="true" focusable="false"><defs>${[...printGeometryRegistry].map(([d,id])=>`<path id="${id}" d="${e(d)}"/>`).join('')}</defs></svg>`:'';
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Territorial diagnostic — ${e(area.name)}</title><style>${diagnosticPrintCss}</style></head><body>${printGeometryDefs}<h1>Territorial diagnostic — ${e(area.name)}</h1><p>${e(territoryLineage(data,territoryId).map(row=>row.name).join(' → '))}</p><p>${e(identityText(area))}</p><p>Selected period ${e(period)} · Data edition ${e(data.generated_at)} · Schema ${e(data.schema_version)}</p><p>This diagnostic describes the selected analysis area. It does not establish a legal planning authority, resident agreement or official approval. Exact whole-area observations have priority. Approved calculations require a complete, non-overlapping cover and remain labelled separately. Recorded statistical differences and hypotheses for local review remain separate.</p>${sections}<h2>Selected-area official materials</h2><div class="material-report">${e(planningDocuments(data,territoryId).map(doc=>documentMarkdown(data,doc)).join('\n\n') || 'No selected-area material collected; plan existence and approval remain unverified.')}</div><h2>Evidence gaps and next actions</h2><ul>${selectedGaps(data,territoryId).map(gap=>`<li>${e(gap.category)} · ${e(gap.status)}: ${e(gap.detail)} Next: ${e(gap.next_action)}</li>`).join('')}</ul><h2>Priority hypotheses and agreements</h2><p>Use these source-grounded differences to prepare questions for local review. Proposed causes, priorities, meeting evidence and approvals must be recorded separately. No such agreement is generated from a colour or rank.</p></body></html>`;
}
