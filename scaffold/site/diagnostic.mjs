import {escapeHtml as e, displayValue, finite, observationState, statusLabel, territoryLineage, mapGeometry, seriesFor, seriesGeometry, observedValue, makeCsv, safeUrl, documentMarkdown} from './model.mjs';
...
const valueText = (data,value) => displayValue(value,data.country.locale || 'en');
...
const overallEntry = (data,area,indicator,period) => {
  const result=observationState(data,area.id,indicator.id,period),context=observationContext(data,area,indicator,result.row);
  return {area,observation:result.row,value:result.value,status:result.status,...context,comparable:null};
};
...
export function comparisonSummary(data,comparison) {
  ...
  return `${values.length} of ${comparison.rows.length} member areas have comparable observations. Range ${valueText(data,minimum)}–${valueText(data,maximum)} ${comparison.indicator.unit}. This range describes recorded differences; it does not establish causes or agreed priorities.`;
}
...
export function renderInternalComparison(data,parentId,indicatorId,period,{interactive=true}={}) {
  ...
  const map=geometry.paths.length?`<svg class="internal-map" viewBox="0 0 760 400" role="group" aria-label="${e(label)}. Inspect values without changing the analysis area."><title>${e(label)}</title><rect width="760" height="400" fill="#f4f8f7"/>${geometry.paths.map(path=>{
    const row=byId.get(path.id),text=`${row.area.name}: ${valueText(data,row.value)} ${rowUnit(row,indicator)} · ${statusLabel(row.status)}${row.comparable?'':' · comparison not established'}`;
...
function seriesReport(data,area,indicator) {
  ...
  return `${graph}<table class="history-table"><thead><tr><th>Period</th><th>Value / unit</th><th>Status / meaning</th><th>Source</th></tr></thead><tbody>${series.map(row=>{
    ...
    return `<tr><td>${e(row.period)}</td><td>${e(valueText(data,observedValue(row)))} ${e(meaning.unit)}</td><td>${e(statusLabel(row.status))}<p>${e(meaningText(meaning))}</p><p>Observation boundary edition: ${e(observedBoundary(row))}.</p></td><td>${sourceLink(source)}<p>Retrieved ${e(source?.retrieved_at || 'not recorded')}.</p></td></tr>`;
  }).join('')}</tbody></table>`;
}
...
export function diagnosticMarkdown(data,territoryId,period) {
  ...
  const lines=[`# Territorial diagnostic — ${md(area.name)}`,'',`Analysis area: ${md(identityText(area))}`,`Hierarchy: ${territoryLineage(data,territoryId).map(row=>md(row.name)).join(' → ')}`,`Selected period: ${md(period)}. Data edition: ${md(data.generated_at)}. Schema: ${md(data.schema_version)}.`, '',
    ...
    'Whole-area values are source-reported observations. Lower-area values are not substituted or averaged to create the total. Different statistical concepts require different indicator IDs or explicitly recorded comparison context.',''];
  for(const indicator of data.indicators) {
    ...
    for(const row of seriesFor(data,territoryId,indicator.id)) {
      ...
      lines.push(`| ${md(row.period)} | ${md(valueText(data,observedValue(row)))} | ${md(meaning.unit)} | ${md(statusLabel(row.status))} | ${md(meaningText(meaning))} | ${md(observedBoundary(row))} | ${md(historySource?.url)} / ${md(historySource?.retrieved_at)} |`);
    }
    if(comparison.set.terminal)lines.push('','Internal comparison stops at this area; no lower areas are included.');
      ...
      for(const row of comparison.rows)lines.push(`| ${md(row.area.name)} / ${md(row.area.id)} | ${md(row.area.type)} / ${md(row.area.code_system)} / ${md(row.area.official_code || 'unverified')} / ${md(row.area.parent_id)} / ${md(row.area.boundary_version || 'unverified')} | ${md(valueText(data,row.value))} | ${md(rowUnit(row,indicator))} / ${md(period)} | ${md(statusLabel(row.status))} / ${row.comparable?'comparable':md(row.reason || 'not comparable')} | ${md(meaningText({...row,reason:''}))} | ${md(observedBoundary(row.observation))} / ${md(row.boundary_reason || 'Matched geometry')} | ${md(row.source?.url)} / ${md(row.source?.retrieved_at)} |`);
