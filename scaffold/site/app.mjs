...
const pilotCountryHeading=()=>language==='ja'?`<span class="pilot-scope-highlight">中米7か国</span>から選ぶ`:language==='es'?`Elige entre los <span class="pilot-scope-highlight">7 países de Centroamérica</span>`:`Choose from the <span class="pilot-scope-highlight">7 Central American countries</span>`;
...
function home() {
  if(worldMode()) {
    ...
    const allCountriesUrl=new URL(`territorial/?${routeWithLanguage({...state,selected:root.id,level:root.level})}`,base).href;
    const allCountriesEntry=`<a class="region-entry pilot-all-entry" href="${e(allCountriesUrl)}"><strong>${e(localCopy('All 7 Central American countries','Los 7 países de Centroamérica','中米7か国すべて'))}</strong><span>${e(localCopy('View the whole region','Ver toda la región','地域全体を見る'))} →</span></a>`;
    return `<section class="home-intro world-intro"><p class="home-lead">Explore population and everyday life through census data. Follow the map from countries to local areas, compare places, and find data for research and regional planning.</p><p class="small-note home-coverage"><strong>Available coverage:</strong> <span>${e(displayAreaName(areaFor(dataset.country.national_territory_id)))}</span><span>. Available topics, years and local detail vary by country.</span></p><div class="actions">${pageLink('territorial','Explore territorial data','button')}${pageLink('thematic','Compare an indicator')}${pageLink('database','Open data database')}</div></section>
    <div class="territorial-top">${mapPanel()}<section class="panel"><h2>${pilotCountryHeading()}</h2><p class="small-note">Start with a place you want to know. Select a country to explore its regions and municipalities.</p><div class="region-entry-grid">${allCountriesEntry}${children.map(area=>{const lower=dataset.territories.filter(item=>item.parent_id===area.id).length;return `<a class="region-entry" href="${e(new URL(`territorial/?${routeWithLanguage({...state,selected:area.id,level:area.level})}`,base).href)}"><strong>${e(area.name)}</strong><span>${lower?`<span>${lower} areas to explore</span>`:'<span>Open country</span>'} →</span></a>`;}).join('')}</div></section></div>
...
function database() {
  ...
  const censusPilot=dataset.analysis?.pilot;
  const currentStage=censusPilot?.census_adapter_status==='partial'
  ...
  <section class="panel"><h2>Build the regional database in stages</h2><ol><li>${e(currentStage)}</li><li>Primary data: official census tables, census years, administrative codes and compatible boundaries for all seven pilot countries.</li><li>Country depth: municipalities and finer planning-analysis geographies, followed by variable harmonization, table builder, extracts, API, boundary history and reproducibility packages.</li></ol></section>`;
}
