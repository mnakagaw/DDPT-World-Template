...
function thematic() {
  ...
  <div class="summary-grid"><article class="summary"><span>${worldMode()?'Selected-area value':'National value'}</span><strong>${fmt(reference.observation.value)}</strong><small>${e(reference.area?.name || state.selected)} · ${e(reference.observation.row?.period || effectivePeriod)} · ${e(referenceMeaning.unit)} · ${e(translateText(statusLabel(reference.observation.status,reference.observation.row),language))}</small></article><article class="summary"><span>${worldMode()?'Median of comparable areas':'Median of comparable local areas'}</span><strong>${fmt(stats.median)}</strong><small>${e(levelLabel(state.level))}; observed values only</small></article><article class="summary"><span>${worldMode()?'Comparison data coverage':'Local data coverage'}</span><strong>${stats.count} / ${rows.length}</strong><small>${rows.length-stats.count} unranked or missing</small></article><article class="summary"><span>${worldMode()?'Observed comparison range':'Observed local range'}</span><strong>${stats.count?`${fmt(stats.min)}–${fmt(stats.max)}`:'No data'}</strong><small>${e(indicator.unit)} · ${e(effectivePeriod)} · same comparison set</small></article></div>
