import {
  ...
  periodsFor, periodForNewMetric, observationState, areaObservationState, thematicReferenceState, effectivePeriodForTerritoryIndicator, territorialIndicatorState, regionalMemberValueSummary, localLevels, levelLabel, nationalOnly, initialState,
  ...
  seriesFor, observedValue, observationDisplayStatus, makeCsv, evidenceCsv, seriesCsv, observationsCsv, comparisonCsv, safeFilename, planningMarkdown,
...
import {renderInternalComparison,diagnosticMarkdown,diagnosticHtml,diagnosticCsv,renderSourceAttribution,seriesSourceLabel,sourceScopeNote} from './diagnostic.mjs';
...
let loadedSupranationalShard=false;
...
const scopeEntryHeading=(children,countryCount)=>dataset.analysis?.pilot?.stage==='central_america_7'
  ...
  : dataset.analysis?.kind==='world'?language==='ja'?`<span class="pilot-scope-highlight">世界</span>の地域から選ぶ`:language==='es'?`Explora las regiones del <span class="pilot-scope-highlight">mundo</span>`:`Explore the <span class="pilot-scope-highlight">world</span> by region`
  : dataset.country.id==='ASI'?language==='ja'?`<span class="pilot-scope-highlight">アジア全域</span>から選ぶ`:language==='es'?`Explora <span class="pilot-scope-highlight">toda Asia</span>`:`Explore <span class="pilot-scope-highlight">all Asia</span>`
...
function stateMessage(result, local = currentArea().level !== 'national') {
  ...
  const explanation = result.status==='not_collected'
    ...
    : localCopy(`This observation is ${statusLabel(result.status,result.provenance).toLowerCase()}.`,`Estado de esta observación: ${translateText(statusLabel(result.status,result.provenance),language)}.`,`この観測値の状態：${translateText(statusLabel(result.status,result.provenance),language)}。`);
...
function metricCard(indicator) {
  ...
  const percentVisual=indicator.visualization==='percent_bar'&&finite(result.value)&&result.value>=0&&result.value<=100?`<figure class="percent-visual" role="img" aria-label="${e(indicator.name)}: ${e(fmt(result.value,indicator))}% ${e(result.row?.period||effectivePeriod)}"><svg class="percent-chart" viewBox="0 0 100 14" preserveAspectRatio="none" aria-hidden="true"><rect width="100" height="14" class="percent-chart-base"/><rect width="${Math.max(0,Math.min(100,result.value))}" height="14" class="percent-chart-value"/></svg><figcaption>0% <span>${e(localCopy('Share of the defined population; not a target','Proporción de la población definida; no es una meta','定義された対象人口の割合。目標達成率ではありません'))}</span> 100%</figcaption></figure>`:'';
  const compactInternationalGap=indicator.series_family==='international_reference'&&currentArea().type!=='country'&&!finite(result.value)&&!partial;
  const members=compactInternationalGap?regionalMemberValueSummary(dataset,state.selected,indicator.id,effectivePeriod):null;
  const memberPeriod=members?.period||effectivePeriod;
  ...
  const overallValue=compactInternationalGap?`<div class="regional-member-summary"><span>${e(memberLabel)}</span>${memberRange}<small>${e(memberCaution)}</small></div>`:`<div class="value-row ${partial?'partial-value':!finite(result.value)?'missing-value':''}"><strong>${fmt(partial?result.covered_value:result.value,indicator)}</strong><span>${partial?`${e(meaning.unit)} · ${e(partialCaution)}`:`${e(translateText(statusLabel(result.status,result.provenance),language))} · ${e(result.row?.period || effectivePeriod || 'No source period')}`}</span></div>${partial?'':stateMessage(result)}`;
...
function territorial() {
  ...
  <div class="territorial-top"><section class="panel selected-profile"><h2>${e(currentArea().name)}</h2>${hierarchyNavigation()}${areaControls()}${identity()}${facts()}</section>${mapPanel()}</div>${countryDetailLink()}${dataset.analysis?.supranational_indicator_ids&&currentArea().type==='exploration_scope'?'':regionalPopulationSummary()}${renderCensusPyramid(dataset,state.selected,language)}${sourceScopeNote(dataset,state.selected,'UN_WPP_POP_TOTAL',language)?`<p class="notice source-scope-note">${e(sourceScopeNote(dataset,state.selected,'UN_WPP_POP_TOTAL',language))}</p>`:''}${renderWppDemographics(dataset,state.selected,state.period,language)}
...
function thematic() {
  ...
  const reference=thematicReferenceState(dataset,state,effectivePeriod);
  const selected=areaObservationState(dataset,state.selected,state.metric,effectivePeriod),selectedMeaning=observationContext(dataset,currentArea(),indicator,selected.row),referenceMeaning=observationContext(dataset,reference.area,indicator,reference.observation.row);
  ...
  <div class="summary-grid"><article class="summary"><span>${worldMode()?'Selected-area value':'National value'}</span><strong>${fmt(reference.observation.value)}</strong><small>${e(reference.area?.name || state.selected)} · ${e(reference.observation.row?.period || effectivePeriod)} · ${e(referenceMeaning.unit)} · ${e(translateText(statusLabel(reference.observation.status,reference.observation.provenance),language))}</small></article><article class="summary"><span>${worldMode()?'Median of comparable areas':'Median of comparable local areas'}</span><strong>${fmt(stats.median)}</strong><small>${e(levelLabel(state.level))}; comparable values only</small></article><article class="summary"><span>${worldMode()?'Comparison data coverage':'Local data coverage'}</span><strong>${stats.count} / ${rows.length}</strong><small>${rows.length-stats.count} unranked or missing</small></article><article class="summary"><span>${worldMode()?'Comparison range':'Local comparison range'}</span><strong>${stats.count?`${fmt(stats.min)}–${fmt(stats.max)}`:'No data'}</strong><small>${e(indicator.unit)} · ${e(effectivePeriod)} · same comparison set</small></article></div>
  ${sourceScopeNote(dataset,state.selected,indicator.id,language)?`<p class="notice source-scope-note">${e(sourceScopeNote(dataset,state.selected,indicator.id,language))}</p>`:''}${!compatibility.comparable?`<p class="notice">${e(compatibility.reason)}</p>`:!stats.count?`<p class="notice"><strong>No comparable local observations for ${e(indicator.name)} · ${e(state.period)}.</strong> ${excluded?`${excluded} published local values are retained but excluded from comparison; see the unranked list for each reason.`:nationalOnly(dataset)?'Local statistics have not yet been collected.':'This level and period have no observed local values for the selected indicator.'} The national source value is shown separately; no local ranking or local estimates are created.</p>`:''}
  ...
  <section class="panel selection-detail"><div><h2>${e(currentArea().name)} — selected area</h2>${identity()}${areaControls()}</div><div><p class="eyebrow">${e(indicator.name)} · ${e(selected.row?.period || effectivePeriod)} · ${e(selectedMeaning.unit)}</p>${!selectedMeaning.comparable?`<p class="notice">${e(selectedMeaning.reason)}</p>`:''}<p class="definition">${e(selectedMeaning.definition)}</p><p class="selected-value">${fmt(selected.value)}</p><p>${e(translateText(statusLabel(selected.status,selected.provenance),language))}. ${currentArea().level==='national'?'National value is not part of the local ranking.':rank?`Rank ${rank} of ${stats.count} comparable areas (${rankOrder==='desc'?'highest':'lowest'} first).`:currentArea().level!==state.level?'Selected area is outside the comparable geographic level.':'No local rank.'}</p>${stateMessage(selected)}${sourceNote(indicator,selected.row,'Source',effectivePeriod)}<div class="actions">${pageLink('territorial','Open territorial diagnostic')}${pageLink('planning','Open planning resources')}${button('comparison-csv','Comparison CSV')}</div></div></section>
...
function home() {
  if(worldMode()) {
    ...
    const wholeLabel=dataset.analysis?.kind==='world'?localCopy(`World — ${countryCount} countries and areas`,`Mundo — ${countryCount} países y áreas`,`世界全体・${countryCount}の国・地域`):dataset.analysis?.pilot?.stage==='central_america_7'?localCopy('All 7 Central American countries','Los 7 países de Centroamérica','中米7か国すべて'):dataset.country.id==='ASI'?localCopy(`All Asia — ${countryCount} countries and areas`,`Toda Asia — ${countryCount} países y áreas`,`アジア全域・${countryCount}の国・地域`):localCopy(`All Americas — ${countryCount} countries and areas`,`Toda América — ${countryCount} países y áreas`,`アメリカ大陸全体・${countryCount}の国・地域`);
    ...
    const coverage=regionalCoverageSummary(dataset);
    const completionNote=dataset.analysis?.kind==='world'?localCopy(`UN WPP has comparable demographic data for ${coverage.un_wpp_country_area_count} of ${countryCount} registered countries and areas. Domestic census detail is added country by country.`,`UN WPP ofrece datos demográficos comparables para ${coverage.un_wpp_country_area_count} de ${countryCount} países y áreas registrados. Los censos subnacionales se incorporan país por país.`,`国連WPPの比較可能な人口統計は${countryCount}の国・地域のうち${coverage.un_wpp_country_area_count}件で利用できます。国内の国勢調査データは国ごとに追加しています。`):localCopy(`${countryCount} countries/areas are available from this regional entry. ${coverageSummaryText(coverage,'en')} Planning materials vary by country or area.`,`${countryCount} países o áreas están disponibles desde esta entrada regional. ${coverageSummaryText(coverage,'es')} Los materiales de planificación varían según el país o área.`,`この広域入口から${countryCount}の国・地域を利用できます。${coverageSummaryText(coverage,'ja')} 計画資料の収録範囲は国・地域によって異なります。`);
    const lead=dataset.analysis?.kind==='world'?localCopy('Explore the world, its regions and countries with comparable UN demographic statistics. Select a country to continue into its available census and local data.','Explore el mundo, sus regiones y países con estadísticas demográficas comparables de la ONU. Seleccione un país para continuar con los censos y datos locales disponibles.','国連の比較可能な人口統計で世界・広域・国をたどれます。国を選ぶと、整備済みの国勢調査と国内地域データへ進めます。'):'Explore population and everyday life through census data. Follow the map from countries to local areas, compare places, and find data for research and regional planning.';
    const scopeLead=dataset.country.id==='ASI'?localCopy('Explore all of Asia through comparable international statistics. Choose a subregion or country, then check its available census and local evidence.','Explora toda Asia con estadísticas internacionales comparables. Elige una subregión o un país y consulta sus censos y datos locales disponibles.','比較可能な国際統計でアジア全域をたどれます。広域や国を選び、収録済みの国勢調査・国内地域データを確認できます。'):lead;
    return `<section class="home-intro world-intro"><p class="home-lead">${e(scopeLead)}</p><p class="small-note home-coverage"><strong>Available coverage:</strong> <span>${e(displayAreaName(areaFor(dataset.country.national_territory_id)))}</span><span>. Available topics, years and local detail vary by country.</span></p><p class="notice compact">${e(completionNote)}</p><div class="actions">${pageLink('territorial','Explore territorial data','button')}${pageLink('thematic','Compare an indicator')}${pageLink('database','Open data database')}</div></section>
...
function database() {
  ...
  const currentStage=scopeCoverage
    ? `${localCopy('Current','Ámbito actual:','現在の収録範囲：')} ${translateText(dataset.country.name,language)}${language==='en'?' scope: ':language==='es'?'. ':'。'}${coverageSummaryText(regionalCoverageSummary(dataset),language)}`
...
async function ensureSupranationalData(){
  const entry=dataset?.data_shards?.supranational;
  if(!entry||loadedSupranationalShard)return;
  const response=await fetch(new URL(entry.file,new URL('data/',base)));
  if(!response.ok)throw new Error(`International data request returned HTTP ${response.status}`);
  const shard=await response.json();
  if(shard.schema_version!=='1.0-supranational-shard'||shard.generated_at!==dataset.generated_at||!Array.isArray(shard.observations))throw new Error('International data edition is incompatible');
  dataset.observations=mergeObservations(dataset.observations,shard.observations);
  dataset.analysis.population_pyramids=shard.population_pyramids||{};
  loadedSupranationalShard=true;
}
...
app.addEventListener('click',async event=>{
  ...
  try {
    ...
    else if(action==='series-csv')download(seriesCsv(dataset,target.dataset.territory,target.dataset.id),`${safeFilename(target.dataset.territory)}-${safeFilename(target.dataset.id)}-history.csv`,'text/csv;charset=utf-8');
    else if(action==='catalog-csv')download(catalogCsv(),`${safeFilename(activeExportId())}-indicator-catalog.csv`,'text/csv;charset=utf-8');
    else if(action==='observations-csv')download(observationsCsv(dataset,activeBranchIds()),`${safeFilename(activeExportId())}-observations.csv`,'text/csv;charset=utf-8');
    ...
    else if(action==='comparison-csv')download(comparisonCsv(dataset,state),`${safeFilename(dataset.country.id+'-'+state.level+'-'+state.metric+'-'+state.period)}-comparison.csv`,'text/csv;charset=utf-8');
