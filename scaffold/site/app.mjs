...
function metricCard(indicator) {
  ...
  const overallValue=compactInternationalGap?`<p class="small-note missing-note">${e(localCopy('No published value for this exact region and year. Available member areas can still be compared below.','No hay un valor publicado para esta región y este año. Puede comparar las áreas con datos más abajo.','この地域・年の公表値はありません。下の表では値のある構成地域を比較できます。'))}</p>`:`<div class="value-row ${partial?'partial-value':!finite(result.value)?'missing-value':''}"><strong>${fmt(partial?result.covered_value:result.value,indicator)}</strong><span>${partial?`${e(meaning.unit)} · ${e(partialCaution)}`:`${e(statusLabel(result.status))} · ${e(result.row?.period || effectivePeriod || 'No source period')}`}</span></div>${partial?'':stateMessage(result)}`;
