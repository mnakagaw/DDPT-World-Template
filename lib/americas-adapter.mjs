...
export function buildAmericas(world,{centralAmerica=null,unPopulation=null}={}){
  ...
  if(unPopulation){
    ...
    if(unPopulation.regional_source||unPopulation.regional_observations){
      if(unPopulation.regional_source?.territory_id!=='M49:005'||unPopulation.regional_source?.geographic_level!=='world_region_series'||unPopulation.regional_source?.sha256!==unPopulation.source.sha256||unPopulation.regional_observations?.length!==4||unPopulation.regional_observations.some(row=>row.territory_id!=='M49:005'||row.indicator_id!==unPopulation.indicator.id||row.source_id!==unPopulation.regional_source.id))throw new Error('Americas UN regional population evidence does not match the South America WPP source.');
      sources.push(clone(unPopulation.regional_source));sourceIds.add(unPopulation.regional_source.id);
      observations.push(...unPopulation.regional_observations.map(clone));
    }
    aggregation={policy:'exact_then_complete_cover',rules:[{indicator_id:'UN_WPP_POP_TOTAL',method:'sum',completeness:'full_cover',period_policy:'same_period',label:'AreaData calculated UN population context',note:'Sum exact country/area observations or exact source-reported subregion observations for one UN WPP period and variant, with every non-overlapping member covered. Missing members remain missing; no partial subtotal is presented as the whole region.'}]};
    if(unPopulation.regional_observations?.length)aggregation.coverage_sets=[{parent_id:'M49:019',indicator_id:'UN_WPP_POP_TOTAL',member_ids:['M49:021','CUSTOM:CAM-CAR','M49:005'],source_ids:['un-m49'],membership_note:'Three disjoint Americas navigation regions cover UN M49 019; South America uses its direct UN WPP subregion observation, Central America + Caribbean and Northern America use full-cover country values.'}];
  }
  ...
  if(centralAmerica){
    ...
    for(const source of centralAmerica.sources)if(!sourceIds.has(source.id)){
      ...
      if(!copy.license){copy.license='Reuse terms not stated in the source publication; raw files are excluded from public redistribution pending terms review.';copy.license_detail='AreaData publishes source-linked factual observations and provenance only. This statement does not assert an open-data license.';copy.terms_review_status='terms_not_stated_in_source';copy.raw_redistribution_status='withheld_pending_terms_review';copy.normalized_observation_publication='source_attributed_factual_observation_only';}
