...
export function validateDataset(data) {
  ...
  for (const s of sources.values()) {
    ...
    if (s.raw_path != null && (!string(s.raw_path) || /(^[/\\]|^[A-Za-z]:|(^|[/\\])\.\.([/\\]|$))/.test(s.raw_path))) fail(`Unsafe raw_path for ${s.id}`);
  ...
  for (const o of data.observations) {
    ...
    if (sources.get(o.source_id)?.geographic_level === 'national' && o.territory_id !== data.country.national_territory_id) fail(`National source ${o.source_id} cannot supply a local observation`);
