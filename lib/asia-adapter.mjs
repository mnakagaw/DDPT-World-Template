...
export function buildAsia(world) {
  ...
  for (const indicatorId of ['UN_AMA_GDP_CURRENT_USD','UN_AMA_GDP_CONSTANT_2020_USD']) {
    if (!indicatorIds.has(indicatorId)) continue;
    const valueFor = id => observations.find(row => row.territory_id === id
      && row.indicator_id === indicatorId && row.period === '2024' && row.status === 'observed')?.value;
    for (const id of ['M49:142','M49:030']) {
      const memberIds = countries.filter(area => id === 'M49:142' || area.parent_id === id).map(area => area.id);
      const sourceValue = valueFor(id),memberValues = memberIds.map(valueFor);
      if (!Number.isFinite(sourceValue) || memberValues.some(value => !Number.isFinite(value)))
        throw new Error(`Cannot verify 2024 UNSD AMA source/member scope for ${id} ${indicatorId}`);
      const listedMemberSum=memberValues.reduce((sum,value)=>sum+value,0);
      scopeDifferences.push({kind:'published_vs_listed_sum_unreconciled',territory_id:id,indicator_id:indicatorId,
        source_id:'unsd-ama-2024',source_scope:'UNSD AMA published Asia/Eastern Asia aggregate',
        comparison_scope:'UN M49 listed country/area rows',reference_period:'2024',source_value:sourceValue,
        listed_member_count:memberIds.length,listed_member_sum:listedMemberSum,difference:sourceValue-listedMemberSum,
        source_locator:'UNSD AMA current-price file 4 or constant-2020 file 8; 2024 column, Asia/Eastern Asia published row. Aggregate reconciliation to listed M49 country rows is not documented in the acquired source.'});
    }
  }
