  ...
  for(const territory of dataset.territories.filter(area=>selectedIds.has(area.id))) {
    ...
    for(const indicator of dataset.indicators) {
      ...
      for(const [label,actual,wanted] of [
        ...
        ['summary status',summary.result.status,expected.status],
        ...
        ['export status',output?.status,expected.status]
      ])if(!same(actual,wanted))errors.push(`${identity}: ${label} ${JSON.stringify(actual)} != ${JSON.stringify(wanted)}`);
      checks.push({territory_id:territory.id,indicator_id:indicator.id,period:String(expected.period),value:expected.value,source_id:expected.source_id,status:expected.status});
      for(const row of rows) {
        const explicit=territorialIndicatorState(dataset,territory.id,indicator.id,String(row.period));
        if(!same(explicit.result.value,row.value)||String(explicit.period)!==String(row.period)||explicit.result.row?.source_id!==row.source_id||explicit.result.status!==row.status)errors.push(`${identity}|${row.period}: explicit period does not reproduce value, period, source and status`);
      }
    }
  }
  return {
