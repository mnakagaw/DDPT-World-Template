...
import {validateWorldCensusListingsAgainstDataset} from './world-census-preflight.mjs';
...
export function validateAnalysis(data) {
  ...
  for(const observation of array(data.observations).filter(object)) {
    ...
    if(observation.status==='observed' && !worldSeriesSourceMatches(data,source,area))fail(source?.geographic_level==='world_region_series'?`World region series ${source.id} cannot supply this regional observation ${observation.territory_id}`:`World country series ${source.id} cannot supply a regional or local observation ${observation.territory_id}`);
  }
  ...
  const incompleteChildCoverIds=strings(config.incomplete_child_cover_ids ?? [],'analysis.incomplete_child_cover_ids');
  for(const id of incompleteChildCoverIds){
    if(!territories.has(id))fail(`Unknown incomplete-child-cover territory ${id}`);
    if(array(config.terminal_territory_ids).includes(id))fail(`Incomplete-child-cover territory ${id} cannot also be terminal`);
  }
  ...
  for(const [index,item] of array(config.comparisons).entries()) {
    ...
    if(incompleteChildCoverIds.includes(item.parent_id))fail(`${path} conflicts with an incomplete child cover declaration`);
    ...
    for(const id of ids) {
      ...
      let ancestor=territories.get(id)?.parent_id;
      while(ancestor&&ancestor!==item.parent_id){
        if(memberSet.has(ancestor)){fail(`${path} has overlapping ancestor/descendant members`);break;}
        ancestor=territories.get(ancestor)?.parent_id;
      }
    }
  ...
  if(own(config,'aggregation')) {
    if(!object(config.aggregation))fail('analysis.aggregation must be an object');
      ...
      if(own(config.aggregation,'coverage_sets') && !Array.isArray(config.aggregation.coverage_sets))fail('analysis.aggregation.coverage_sets must be an array');
      const covers=new Set();
      for(const [index,cover] of array(config.aggregation.coverage_sets).entries()) {
        const path=`analysis.aggregation.coverage_sets[${index}]`;
        if(!object(cover)){fail(`${path} must be an object`);continue;}
        const parent=territories.get(cover.parent_id),rule=array(config.aggregation.rules).find(item=>item.indicator_id===cover.indicator_id);
        if(!parent)fail(`${path}.parent_id must identify a known territory`);
        if(rule?.method!=='sum')fail(`${path}.indicator_id requires an approved sum rule`);
        const key=`${cover.parent_id}/${cover.indicator_id}`;
        if(covers.has(key))fail(`${path} duplicates ${key}`);covers.add(key);
        const members=strings(cover.member_ids,`${path}.member_ids`,{nonempty:true});
        const children=array(data.territories).filter(area=>area.parent_id===cover.parent_id).map(area=>area.id);
        if(members.length!==children.length||new Set(members).size!==members.length||children.some(id=>!members.includes(id)))fail(`${path}.member_ids must be every direct child exactly once`);
        if(!text(cover.membership_note))fail(`${path}.membership_note must be nonempty`);
        for(const id of strings(cover.source_ids,`${path}.source_ids`,{nonempty:true})) {
          if(!sources.has(id))fail(`${path} has unknown membership source ${id}`);
          else if(!['ready','partial'].includes(sources.get(id).status))fail(`${path} membership source ${id} must have usable evidence`);
        }
      }
    }
  }
  ...
  errors.push(...validateWorldCensusListingsAgainstDataset(data));
