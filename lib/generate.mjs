...
export async function generateSite({dataset, outDir, canonicalSha256=null}) {
  ...
  for(const file of ['app.mjs','model.mjs','analysis.mjs','aggregation.mjs','census-history.mjs','diagnostic.mjs','i18n.mjs','planning.mjs','planning-view.mjs','un-demographics.mjs','styles.css']) {
  ...
  if(['world','regional'].includes(dataset.analysis?.kind)&&descendantCountryIds.size){
    const internationalIds=new Set(dataset.analysis?.supranational_indicator_ids||[]);
    const indicatorById=new Map(dataset.indicators.map(indicator=>[indicator.id,indicator]));
    ...
    for(const countryId of [...descendantCountryIds].sort()){
      ...
      const shard={schema_version:'0.2-country-shard',country_area_id:countryId,generated_at:dataset.generated_at,
        ...
        observations:dataset.observations.filter(row=>lowerIds.has(row.territory_id)||row.territory_id===countryId&&indicatorById.get(row.indicator_id)?.series_family!=='international_reference'),
    ...
    publicDataset.observations=dataset.observations.filter(row=>!descendantIds.has(row.territory_id)&&!(descendantCountryIds.has(row.territory_id)&&indicatorById.get(row.indicator_id)?.series_family!=='international_reference'));
    ...
    const includeFullDataset=dataset.analysis?.kind!=='world';
    ...
    publicDataset.data_shards={schema_version:'1.1',base:'data/',countries:shardManifest,...(includeFullDataset?{full:'dashboard-full.json'}:{}),integrity:{canonical_sha256:canonicalSha256,full_sha256:sha256(fullContent),territory_count:dataset.territories.length,observation_count:dataset.observations.length,boundary_count:dataset.boundaries?.features?.length||0,document_count:dataset.documents?.length||0,comparison_count:dataset.analysis?.comparisons?.length||0,countries:countryIntegrity},note:'Country-depth data load when a country or one of its lower areas is selected. The canonical full dataset remains in the project outside the public site.'};
    if(internationalIds.size){
      const payload={schema_version:'1.0-supranational-shard',generated_at:dataset.generated_at,indicator_ids:[...internationalIds],observations:publicDataset.observations.filter(row=>internationalIds.has(row.indicator_id)),population_pyramids:dataset.analysis?.population_pyramids||{}};
      const content=JSON.stringify(payload)+'\n',relative='supranational.json',filename=path.join(siteDir,'data',relative);
      await writeFile(filename,content,'utf8');files.push(filename);
      publicDataset.observations=publicDataset.observations.filter(row=>!internationalIds.has(row.indicator_id));
      delete publicDataset.analysis.population_pyramids;
      publicDataset.data_shards.supranational={file:relative,sha256:sha256(content),observation_count:payload.observations.length,age_profile_count:Object.keys(payload.population_pyramids).length};
    }
  }
