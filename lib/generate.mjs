  ...
  if(['world','regional'].includes(dataset.analysis?.kind)&&descendantCountryIds.size){
    ...
    for(const countryId of [...descendantCountryIds].sort()){
      ...
      const shard={schema_version:'0.2-country-shard',country_area_id:countryId,generated_at:dataset.generated_at,
        ...
        boundaries:{type:'FeatureCollection',features:(dataset.boundaries?.features||[]).filter(feature=>lowerIds.has(feature.properties?.territory_id))},
      ...
      countryIntegrity[countryId]={sha256:sha256(shardContent),territory_count:shard.territories.length,observation_count:shard.observations.length,boundary_count:shard.boundaries.features.length,document_count:shard.documents.length,comparison_count:shard.comparisons.length};
    }
  ...
  const lines=[
    ...
    ...dataset.gaps.map(gap=>{
      if(dataset.analysis?.kind==='regional'&&gap.category==='census_country_coverage'){
        ...
        const structural=dataset.analysis?.coverage?.census_structurally_not_applicable_country_ids||[];
        const structuralNote=structural.length?` The remaining ${structural.length} (${structural.join(', ')}) are evidence-backed structural exceptions.`:'';
        return `- ${gap.category} (${integrated+structural.length===total?'ready':'partial'}): Official Census or equivalent population evidence and available domestic hierarchy are integrated for ${integrated} of ${total} registry entries.${structuralNote} Missing or structurally inapplicable evidence remains distinct from zero. Next: Continue country-specific source updates without filling evidence gaps with estimates.`;
      }
